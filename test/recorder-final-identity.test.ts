...
it.each(['question', 'interim', 'final'] as const)('keeps completion across historical %s metadata revisions and restart', async revised => {
  ...
  if (revised === 'final') {
    ...
    // Unowned historical prose is not new work under the official recorder contract.
    // A real new generation carries its own start and must invalidate the prior final.
    await recordChatObservations(conversationId, revised === 'question'
      ...
      : [{ kind: 'turn_start', time: 8, turnId: 'new-live-turn' },
        { kind: 'assistant_message', time: 9, turnId: 'new-live-turn', messageId: 'new-interim', text: 'New work', state: 'streaming', activeNow: true }]);
    expect(await readCompletedFinal(id, conversationId, 'current-turn')).toBeNull();
  }
});
